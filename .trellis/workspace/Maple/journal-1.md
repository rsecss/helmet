...
| 项目 | 内容 |
|---|---|
| 功能 | 新增 `APP/fan_control.c/.h` 风扇仲裁模块，DHT11 `temp>=30°C` 自动启动 1 档，`temp<=28°C` 恢复。 |
| 优先级 | Web/语音 `motor_speed_0` 为最高优先级关闭指令，高温自动打开期间也会立即停机；`motor_speed_1..3` 解除关闭覆盖。 |
| 协议 | 4G telemetry 新增 `fan_auto`、`temp_limit`、`temp_recover`，`motor` 改为最终输出档位。 |
| 接入 | Web/M100PG 与 ASRPro motor 命令改走 `fan_control_set_manual_gear()`；新增模块登记到 `bsp_system.h`、`scheduler.c`、`main.c`、`helmet.uvprojx`。 |
| 文档 | 更新 README、CLAUDE、CHANGELOG、`.trellis/spec/backend/` 可执行合同和任务 PRD。 |
| 验证 | 用户完成实机测试；本地通过 `git diff --check`、触及文件编码检查、APP 头文件守卫、GCC 语法检查、Keil 工程 XML 解析。 |
| 提交 | `f7a85db feat(fan): 接入高温自动风扇仲裁`；任务归档 `02b024b chore(task): 归档 05-18-temp-threshold-fan-auto`。 |

**残留工作区说明**
- `MDK-ARM/helmet.uvoptx` 为 IDE 用户选项改动，未纳入功能提交。
- `工程设计与代码约束.md` 为用户提供的约束文档，未纳入功能提交。
