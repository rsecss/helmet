...
- **禁止** 在 Keil 会直接编译解析的运行时 C 字符串或字体表键中写裸中文/乱码中文（如 `"温度"`、`"锟斤拷"`、`"娣?"`）。需要显示中文时，运行时字符串使用 UTF-8 十六进制转义，字体索引用 UTF-8 字节数组，避免不同源码字符集触发 `missing closing quote` 或数组宽度错误。
...
- **LCD 中文文本**：`APP/lcd_app.c` 的中文显示标签必须用 UTF-8 hex escape 写法，例如 `"\xE6\xB8\xA9\xE5\xBA\xA6"`，不要依赖 Keil 对源文件中文字符集的识别。`APP/lcd_font_lib.h` 的中文点阵只保留项目实际使用字符，索引使用 `unsigned char Index[4]` 存三字节 UTF-8 + NUL。
...
- [ ] LCD 中文显示改动无 Keil `missing closing quote`、`char[n] cannot initialize unsigned char[...]` 字体表报错
...
- [ ] 4 空格缩进、大括号风格符合约定
- [ ] LCD 中文运行时字符串使用 UTF-8 hex escape，字体表键使用 UTF-8 字节数组，无 `锟斤拷` / `娣?` / `鍦?` 等乱码字面量
- [ ] 12px 中文字模实机确认同高显示，`温` / `度` / 带顶点或顶横字符没有顶部裁切
